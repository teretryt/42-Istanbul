#include "../../include/bot/Commands.hpp"
...
Commands::Commands(std::string userName, std::string nickName)
:nickName(nickName), userName(userName){
	funcMap["!help"] = &Commands::cmdHelp;
	funcMap["!cmd"] = &Commands::cmdCmd;
...
void	Commands::runComs(std::string coms){
	parsed = parser(coms);
	for (size_t i = 0; i < parsed.size(); i++)
	{
		...
		if (splitted[1] == "PRIVMSG"){
			std::map<std::string, void (Commands::*)()>::iterator it = this->funcMap.find(splitted[3].erase(0, 1));
...
std::string Commands::parseSenderName(){
...
//SET FUNCS
void	Commands::setSocket(int socket){ this->_socket = socket; }

//GET FUNCS
int	Commands::getSocket() const { return (_socket); }
